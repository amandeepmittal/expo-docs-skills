---
name: docs-boxlink-audit
version: 2.0.0
description: Audit `<BoxLink>` icons in an Expo docs PR against the destination-to-icon conventions and stage findings as a private pending GitHub review (never auto-submitted). Use when the user provides a public PR URL and asks to "audit boxlink icons", "review boxlinks", or "check boxlink icons". Icon mismatches only.
argument-hint: <pr-url>
allowed-tools: Read, Grep, Glob, Write, Bash(gh:*), Bash(bun:*)
---
...
`$ARGUMENTS` is a public GitHub PR URL. Accepted forms:
...
- **`references/boxlink-icons.md`** — canonical destination→icon mapping, severity guidance, special-case exceptions.
...
4. **Match each in-scope BoxLink against the canonical mapping** in `references/boxlink-icons.md`. For each mismatch, classify per the reference's severity guidance:
...
- The rule ref: `boxlink-icons.md#canonical-mapping`.
- The exact fix: name the canonical icon and the import path. The icon must exist: verify the file under `expo/docs/node_modules/@expo/styleguide-icons/<variant>/<Name>.js` (or that it comes from `@expo/styleguide`) before suggesting it.
...
- `/tmp/docs-boxlink-audit-pr-{number}-{file-slug}.json` (source of truth, machine-readable)
- `/tmp/docs-boxlink-audit-pr-{number}-{file-slug}.md` (human-readable, rendered from the JSON)
  ...
  "comments": [
    {
      ...
      "severity": "design",
      "rule_ref": "boxlink-icons.md#canonical-mapping",
      "body": "**[design]** This BoxLink points at `/eas/build/`; use `BuildIcon` rather than `BookOpen02Icon` for EAS Build links (`boxlink-icons.md#canonical-mapping`).\n\n```suggestion\n  Icon={BuildIcon}\n```\n\nAlso update the import to `import { BuildIcon } from '@expo/styleguide-icons/custom/BuildIcon';`.",
      "resolved": false
    }
  ]
}
...
2. Run the script with all paths in one invocation:
   ...
   ```sh
   bun skills/review/scripts/post-review.ts \
     /tmp/docs-boxlink-audit-pr-{number}-*.json
   ```
...
1. Read the prior JSON at `/tmp/docs-boxlink-audit-pr-{number}-*.json`.
