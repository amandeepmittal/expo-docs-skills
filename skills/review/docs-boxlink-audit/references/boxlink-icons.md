...
| External library documentation sites (`docs.swmansion.com`, `shopify.github.io`, `reactnative.dev`, `clerk.com`, `posthog.com`, and similar) | `BookOpen02Icon` | `@expo/styleguide-icons/outline/BookOpen02Icon` | High (2026-06-10 sweep). A matching brand icon wins when one exists in the set (`GithubIcon`, `SentryIcon`, `FigmaIcon`). |
| `developer.apple.com` | `AppleAppStoreIcon` | `@expo/styleguide-icons/custom/AppleAppStoreIcon` | 2 of 3; `BookOpen02Icon` tolerated. |
...
- **Missing `Icon` prop entirely.** Out of scope; ~30 of 669 BoxLinks ship without an icon (common on `supabase.com`, `firebase.google.com`, and `expo.fyi` links). This audit flags wrong icons, not absent ones.
...
Counts come from a sweep of 669 `<BoxLink>` usages across `expo/docs/pages/` on 2026-06-10. Every canonical icon in the table was verified to exist in the installed `@expo/styleguide-icons` package on that date. When suggesting any icon not in this table, verify its file exists under `expo/docs/node_modules/@expo/styleguide-icons/<variant>/` first.
